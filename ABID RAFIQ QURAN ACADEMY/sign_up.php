...
if((empty($_POST['name']))  || (empty($_POST['mobile'])) || (empty($_POST['address']))
    ...
    if ( ($_POST['pass1'] == $_POST['pass2']) ) {
        ...
        $rw = $res->fetch_assoc();
            ...
            $fullName = $_POST['name'] . " (S/O " . $_POST['father_name'] . ")";

            $Qar ="INSERT INTO supervisor (CENTER_NUMBER, ID, BIRTHDATE, ADDRESS, PHONE_NUMBER, PASS, NAME_SUP, Email) VALUES ('".$_POST['mrqz']."','".++$rw['MAX(ID)']."', '".$_POST['georgian_start_date']."', '".$_POST['address']."', '".$_POST['mobile']."', '".$_POST['pass1']."', '".$fullName."', '".$_POST['email']."');";
            $res = $db->query($Qar);
            $db ->close();
$flagSamePass = 2;

        } elseif ($_COOKIE['selectItim'] == 1) { //// ReigsterStudent
            ...
            if (isset($_POST['comboSuper'])) {
                ...
                $Qar = "INSERT INTO students (CENTER_NUMBER, ST_ID, BIRTHDATE, ADDRESS, PHONE_NUMBER, PASS, NAME_STUDENT, Email,ST_SUP) VALUES ('" . $_POST['mrqz'] . "','" . ++$rw['MAX(ST_ID)'] . "', '" . $_POST['georgian_start_date'] . "', '" . $_POST['address'] . "', '" . $_POST['mobile'] . "', '" . $_POST['pass1'] . "', '" . $fullName . "', '" . $_POST['email'] . "', '" . $_POST['comboSuper'] . "');";
                $res = $db->query($Qar);
                $db->close();
        ...
        } elseif ($_COOKIE['selectItim'] == 3) { //// Info
             $flagSamePass = 2;
             $yourID = "Info Request Received";
        }
...
<head>
    <meta charset="UTF-8">
    <title>Abid Rafiq Quran Academy - Sign Up</title>
...
document.cookie = "selectItim="+jstTophp;
  ...
  function handleTypeChange(val) {
    if (val === "student") {
      ...
      document.cookie = "selectItim=2";
    } else {
      // Info
      document.getElementById("comboSuper").style.visibility = "hidden";
      document.getElementById("labSuper").style.visibility = "hidden";
      document.cookie = "selectItim=3";
    }
  }
  ...
  <div class="row align-items-stretch">
    ...
    <div class=" col-12 col-md-4 bgg-color position-md-sticky d-flex align-items-center vh-md-100" style="top: 0; ">
      <div class="card-body px-md-0 px-lg-5">
        <div class="justify-content-center align-items-center h-100 py-3 text-center">
          <div class="mb-4">
              <img src="img/logo.png" alt="Logo" style="width: 150px; filter: drop-shadow(0 5px 15px rgba(0,0,0,0.2));">
          </div>
          <h1 class="display-4 fw-bolder color01 mt-3">يا مرحبًا بأهل القرآن</h1>
        ...
        <form  id="plan_form" action="sign_up.php" method="POST" >
          ...
          <div class="row clearfix mt-md-12 mt-6 mb-md-8 mb-4">
            ...
            <div class="col-12 mb-4">
              ...
              <label class="my-1">الاسم الثلاثي (Full Name)</label>
              <input type="text" name="name" id="path-registration-first-name" class="sm-form-control border-form-control col-12 col-md-8" required="required" placeholder="Full Name">

              <div style="padding: 20px 0 0 0 ">
                <label class="my-1">اسم الأب (Father Name)</label>
                <input type="text" name="father_name" id="father_name" class="sm-form-control border-form-control col-12 col-md-8" required="required" placeholder="Father Name">
              </div>

              <div style="padding: 20px 0 0 0 ">
              <label class="my-1">رقم الهاتف (Contact No)</label>
              <div class="d-flex col-12 col-md-8 px-0">
                  <select name="country_code" class="sm-form-control border-form-control" style="width: 120px; margin-inline-end: 10px;">
                  ...
                  <input type="text" name="mobile" id="mobile_number" class="sm-form-control border-form-control flex-grow-1" required="required" placeholder="Mobile Number">
              </div>
              </div>
...
?>
          ...
          <div class="col-12 mb-4">
            <label class="my-1">طبيعة المسجل (Registration Type)</label>
            <select name="user_type" id="user_type" class="sm-form-control border-form-control col-12 col-md-8" onchange="handleTypeChange(this.value)">
                <option value="supervisor">مشرف (Teacher)</option>
